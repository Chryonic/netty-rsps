...
public class Rasterizer extends DrawingArea {
	...
	public static void method368(CacheArchive streamLoader)
	{
		...
		for(int j = 0; j < 50; j++)
			try
			{
				textureSprite[j] = new IndexedImage(streamLoader, String.valueOf(j), 0);
				if(lowMem && textureSprite[j].imageWidth == 128)
					textureSprite[j].load128x128Image();
				else
					textureSprite[j].load64x64Image();
				anInt1473++;
			}
			catch(Exception _ex) { }
	...
	public static int getAverageRgbColourForTexture(int i)
	{
	...
	private static int[] method371(int i)
	{
		...
		} else
		{
			...
			for(int l = 0; l < anInt1473; l++)
				if(anIntArrayArray1479[l] != null && (textureID[l] < j || k == -1))
				{
					j = textureID[l];
					k = l;
				}
		...
		IndexedImage background = textureSprite[i];
	...
	public static void method372(double d)
	{
		...
		for(int k = 0; k < 512; k++)
		{
			...
			for(int k1 = 0; k1 < 128; k1++)
			{
				...
				if(k2 == 0)
					k2 = 1;
				hslToRgbLookupTable[j++] = k2;
			}

		}

		for(int l = 0; l < 50; l++)
			if(textureSprite[l] != null)
			{
				int ai[] = textureSprite[l].imageColors;
	...
	public static IndexedImage textureSprite[] = new IndexedImage[50];
	...
	public static int textureID[] = new int[50];
	public static int anInt1481;
	public static int hslToRgbLookupTable[] = new int[0x10000];
