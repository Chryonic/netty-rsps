...
public class Censor {
	...
	{
		RSBuffer stream = new RSBuffer(streamLoader.getDataForName("fragmentsenc.txt"));
		RSBuffer stream_1 = new RSBuffer(streamLoader.getDataForName("badenc.txt"));
		RSBuffer stream_2 = new RSBuffer(streamLoader.getDataForName("domainenc.txt"));
		RSBuffer stream_3 = new RSBuffer(streamLoader.getDataForName("tldlist.txt"));
		readValues(stream, stream_1, stream_2, stream_3);
	}

	private static void readValues(RSBuffer stream, RSBuffer stream_1, RSBuffer stream_2, RSBuffer stream_3)
	{
		readBadEnc(stream_1);
	...
	private static void readTldList(RSBuffer stream)
	{
		...
		for(int j = 0; j < i; j++)
		{
			tldArray[j] = stream.readUnsignedByte();
	...
	private static void readBadEnc(RSBuffer stream)
	{
		int j = stream.readDWord();
		badenc = new char[j][];
		badencByte = new byte[j][][];
		method493(stream, badenc, badencByte);
	}

	private static void readDomainEnc(RSBuffer stream)
	{
		int i = stream.readDWord();
		domainenc = new char[i][];
			method494(domainenc, stream);
	}

	private static void readFragmentsEnc(RSBuffer stream)
	{
	...
	private static void method493(RSBuffer stream, char ac[][], byte abyte0[][][])
	{
	...
	private static void method494(char ac[][], RSBuffer stream)
	{
		for(int j = 0; j < ac.length; j++)
		{
			char ac1[] = new char[stream.readUnsignedByte()];
	...
	private static void method500(char ac[])
	{
		for(int i = 0; i < 2; i++)
		{
			for(int j = badenc.length - 1; j >= 0; j--)
				method509(badencByte[j], ac, badenc[j]);
