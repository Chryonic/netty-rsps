...
import com.runescape.IndexedImage;
import com.runescape.RSInterface;
import com.runescape.Rasterizer;
import com.runescape.TextClass;
...
public class Chat {
	...
	private void drawChatArea(GameClient client)
	{
		...
		} else
		{
			...
			for(int k = 0; k < 100; k++)
				if(client.chatMessages[k] != null)
				{
					...
					if((l == 3 || l == 7) && client.splitPrivateChat == 0 && (l == 7 || privateChatMode == 0 || privateChatMode == 1 && isFriendOrSelf(s1)))
					{
						if(i1 > 0 && i1 < 110)
						{
							int k1 = 4;
			...
			if(anInt1211 < 78)
				anInt1211 = 78;
			manageChatInterface(77, anInt1211 - anInt1089 - 77, 0, 463, anInt1211);
			String s;
				...
				if(j == 13 || j == 10)
				{
					...
					if(friendsListAction == 3 && promptInput.length() > 0)
					{
						...
						if(privateChatMode == 2)
						{
							...
							stream.putLEShort(publicChatMode);
							stream.putLEShort(privateChatMode);
	...
	public void pushMessage(GameClient client, String s, int i, String s1)
	{
	...
	private void manageChatInterface(GameClient client, int j, int k, int l, int i1, int j1)
	{
	...
	private void processChatModeClick(GameClient client)
	{
		if(client.clickMode3 == 1)
		{
			if(client.saveClickX >= 6 && client.saveClickX <= 106 && client.saveClickY >= 467 && client.saveClickY <= 499)
			{
				...
				client.stream.putLEShort(publicChatMode);
				client.stream.putLEShort(privateChatMode);
			...
			{
				client.privateChatMode = (privateChatMode + 1) % 3;
				aBoolean1233 = true;
				client.inputTaken = true;
				client.stream.putPacketID(95);
				client.stream.putLEShort(publicChatMode);
	...
	private void resetSpokenText(GameClient client)
	{
