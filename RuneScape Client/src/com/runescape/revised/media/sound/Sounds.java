package com.runescape.revised.media.sound;
...
final class Sounds {
	...
	private Sounds()
	{
		aClass6Array329 = new Sythesizer[10];
	}

	public static void unpack(RSBuffer stream)
	{
		aByteArray327 = new byte[0x6baa8];
		aStream_328 = new RSBuffer(aByteArray327);
		Sythesizer.method166();
		do
		{
	...
	public static RSBuffer method241(int i, int j)
	{
	...
	private void method242(RSBuffer stream)
	{
		for(int i = 0; i < 10; i++)
		{
			...
			if(j != 0)
			{
				...
				aClass6Array329[i] = new Sythesizer();
	...
	private RSBuffer method244(int i)
	{
		int k = method245(i);
	...
	private static RSBuffer aStream_328;
	private final Sythesizer[] aClass6Array329;
