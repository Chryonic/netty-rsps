...
public final class GFX {
	...
	{
		RSBuffer stream = new RSBuffer(streamLoader.getDataForName("spotanim.dat"));
		int length = stream.readUnsignedWord();
		if(cache == null)
			cache = new GFX[length];
	...
	{
		RSModel model = (RSModel) aMRUNodes_415.insertFromCache(anInt404);
		if(model != null)
			return model;
		model = RSModel.method462(anInt405);
		if(model == null)
			return null;
		for(int i = 0; i < 6; i++)
